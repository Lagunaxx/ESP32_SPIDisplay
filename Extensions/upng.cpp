...
namespace Device {
	namespace Display {
		namespace uPNG {
extern upng_error uz_inflate(unsigned char *out, unsigned long outsize, unsigned char *in, unsigned long insize);
...
#define upng_chunk_critical(chunk) (((chunk)[4] & 32) == 0)
			...
			void unfilter_scanline(upng_t *upng, unsigned char *recon, unsigned char *scanline, unsigned char *precon, unsigned long bytewidth, unsigned char filterType, unsigned long length)
			{
				/*
			...
			void unfilter(upng_t *upng, unsigned char *out, unsigned char *in, unsigned w, unsigned h, unsigned bpp)
			{
				/*
					For PNG filter method 0
			...
			void remove_padding_bits(unsigned char *out, unsigned char *in, unsigned long olinebits, unsigned long ilinebits, unsigned h)
			{
				/*
			...
			void c_uPNG::post_process_scanlines(upng_t *upng, unsigned char *out, unsigned char *in, upng_t *info_png)
			{
			...
			upng_format determine_format(upng_t *upng)
			{
			...
			void upng_free_source(upng_t *upng)
			{
			...
			void upng_setup_for_single_image(upng_t *upng)
			{
			...
			upng_error upng_process_chunks(upng_t* upng)
			{
			...
			upng_error c_uPNG::upng_header(upng_t *upng)
			{
			...
			upng_error c_uPNG::upng_decode(upng_t *upng)
			{
			...
			upng_t *c_uPNG::upng_new_from_source(upng_source source)
			{
				upng_t *upng;
...
///
			unsigned long upng_byte_source_read(void* user, unsigned long offset, void* out_buffer, unsigned long read_size)
			{
			...
			void upng_byte_source_free(void* user)
			{
				app_free(user);
			}

			upng_t *c_uPNG::upng_new_from_bytes(unsigned char *raw_buffer, unsigned long size, uint8_t **out_buffer)
			{
			...
			unsigned long upng_file_source_read(void* user, unsigned long offset, void* out_buffer, unsigned long read_size)
			{
				File *fp= (File*)user;
				//FILE* fp = (FILE*)user;
			 ...
			 void upng_file_source_free(void* user)
			{
				if (user != NULL)
					((File*)user)->close();
			}

			upng_t *c_uPNG::upng_new_from_file(char *filename)
			{
				File *fp=new(File);
			...
			void c_uPNG::upng_free(upng_t *upng)
			{
			...
			upng_error c_uPNG::upng_get_error(upng_t *upng)
			{
				return upng->error;
			}

			unsigned c_uPNG::upng_get_error_line(upng_t *upng)
			{
				return upng->error_line;
			}

			unsigned c_uPNG::upng_get_width(upng_t *upng)
			{
				return upng->width;
			}

			unsigned c_uPNG::upng_get_height(upng_t *upng)
			{
				return upng->height;
			}

			int c_uPNG::upng_get_x_offset(upng_t *upng)
			{
				return upng->x_offset;
			}

			int c_uPNG::upng_get_y_offset(upng_t *upng)
			{
				return upng->y_offset;
			}

			int c_uPNG::upng_get_palette(upng_t *upng, rgb **palette)
			{
				*palette = upng->palette;
				return upng->palette_entries;
			}

			int c_uPNG::upng_get_alpha(upng_t *upng, uint8_t **alpha)
			{
				*alpha = upng->alpha;
				return upng->alpha_entries;
			}

			unsigned c_uPNG::upng_get_bpp(upng_t *upng)
			{
				return upng_get_bitdepth(upng) * upng_get_components(upng);
			}

			unsigned c_uPNG::upng_get_components(upng_t *upng)
			{
				switch (upng->color_type)
				{
				case UPNG_PLT:
			...
			unsigned c_uPNG::upng_get_bitdepth(upng_t *upng)
			{
				return upng->color_depth;
			}

			upng_format c_uPNG::upng_get_format(upng_t *upng)
			{
				return upng->format;
			}

			char* c_uPNG::upng_get_text(upng_t *upng, char **text_out, unsigned int index)
			{
			...
			unsigned char *c_uPNG::upng_get_buffer(upng_t *upng)
			{
				return upng->buffer;
			}

			unsigned c_uPNG::upng_get_size(upng_t *upng)
			{
				return upng->size;
			}
			...
			***************************************************************************************/
			...
			void c_uPNG::DrawFile(char *filename, T_DispCoords x, T_DispCoords y)
			{
				...
				if (upng_get_error(upng) != UPNG_EOK) {
					...
					Device::Display::Graphics::Graph->drawImageBufferAlpha(x,y,tmpbuffer,tmpalpha,width, height);// yy+=height+5;//ToDo: make buffer converting
					free(tmpalpha);
					free(tmpbuffer);
/*
