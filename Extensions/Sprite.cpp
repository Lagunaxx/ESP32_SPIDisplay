...
namespace Device{
	namespace Display{
		...
		*************************************************************************************x*/
		...
		void* TFT_eSprite::createSprite(_CoordsType w, _CoordsType h, uint8_t frames)
		{
		...
		*************************************************************************************x*/
		...
		void* TFT_eSprite::callocSprite(_CoordsType w, _CoordsType h, uint8_t frames)
		{
		...
		*************************************************************************************x*/
		void TFT_eSprite::setPivot(_CoordsType x, _CoordsType y)
		{
		  _xpivot = x;
		  _ypivot = y;
		}
		...
		***************************************************************************************/
		_CoordsType TFT_eSprite::getPivotX(void)
		{
		  return _xpivot;
		}
		...
		***************************************************************************************/
		_CoordsType TFT_eSprite::getPivotY(void)
		{
		  return _ypivot;
		}
...
/*		  if ( !_created ) return false;
		  ...
		  _tft->SPIStartWrite(); // ESP32: avoid transaction overhead for every tft pixel

		  // Scan destination bounding box and fetch transformed pixels from source Sprite
		  for (int32_t x = min_x; x <= max_x; x++) {
			int32_t xt = x - _tft->__xpivot();
			float cxt = cosra * xt + _xpivot;
			float sxt = sinra * xt + _ypivot;
			bool column_drawn = false;
			for (int32_t y = min_y; y <= max_y; y++) {
			  int32_t yt = y - _tft->__ypivot();
		  ...
		  _tft->SPIEndWrite(); // ESP32: end transaction
*/
		  return true;
		}
		...
		*************************************************************************************x*/
		void TFT_eSprite::getRotatedBounds(float sina, float cosa, _CoordsType w, _CoordsType h, _CoordsType xp, _CoordsType yp,
				_CoordsType *min_x, _CoordsType *min_y, _CoordsType *max_x, _CoordsType *max_y)
		{
		  ...
		  _CoordsType x0 = -xp * cosa - yp * sina;
		  _CoordsType y0 =  xp * sina - yp * cosa;

		  _CoordsType x1 =  w * cosa - yp * sina;
		  _CoordsType y1 = -w * sina - yp * cosa;

		  _CoordsType x2 =  h * sina + w * cosa;
		  _CoordsType y2 =  h * cosa - w * sina;

		  _CoordsType x3 =  h * sina - xp * cosa;
		  _CoordsType y3 =  h * cosa + xp * sina;
		...
		*************************************************************************************x*/
		void TFT_eSprite::pushSprite(_CoordsType x, _CoordsType y)
		{
/*		  if (!_created) return;
...
*/		}
		...
		*************************************************************************************x*/
		void TFT_eSprite::pushSprite(_CoordsType x, _CoordsType y, uint16_t transp)
		{
/*		  if (!_created) return;
...
*/		}
		...
		*************************************************************************************x*/
		uint16_t TFT_eSprite::readPixel(_CoordsType x, _CoordsType y)
		{
		...
		*************************************************************************************x*/
		void  TFT_eSprite::pushImage(_CoordsType x, _CoordsType y, _CoordsType w, _CoordsType h, uint16_t *data)
		{
		...
		*************************************************************************************x*/
		void  TFT_eSprite::pushImage(_CoordsType x, _CoordsType y, _CoordsType w, _CoordsType h, const uint16_t *data)
		{
		#ifdef ESP32
		...
		*************************************************************************************x*/
		void TFT_eSprite::setWindow(_CoordsType x0, _CoordsType y0, _CoordsType x1, _CoordsType y1)
		{
		...
		*************************************************************************************x*/
		void TFT_eSprite::setScrollRect(_CoordsType x, _CoordsType y, _CoordsType w, _CoordsType h, uint16_t color)
		{
		...
		*************************************************************************************x*/
		void TFT_eSprite::scroll(_CoordsType dx, _CoordsType dy)
		{
		  ...
		  _CoordsType w  = _sw - abs(dx); // line width to copy
		  _CoordsType h  = _sh - abs(dy); // lines to copy
		  _CoordsType iw  = _iwidth;       // width of sprite

		  // Fetch the x,y origin set by setScrollRect()
		  _CoordsType tx = _sx; // to x
		  _CoordsType fx = _sx; // from x
		  _CoordsType ty = _sy; // to y
		  _CoordsType fy = _sy; // from y
		  ...
		  _CoordsType fyp = fx + fy * _iwidth;
		  _CoordsType typ = tx + ty * _iwidth;
		...
		*************************************************************************************x*/
		...
		//void TFT_eSprite::setCursor(_CoordsType x, _CoordsType y)
		//{
		//  this->cursor_x = x;
		//  this->cursor_y = y;
		//}
		...
		*************************************************************************************x*/
		...
		_CoordsType TFT_eSprite::width(void)
		{
		...
		*************************************************************************************x*/
		_CoordsType TFT_eSprite::height(void)
		{
		  if (!_created ) return 0;
		...
		*************************************************************************************x*/
		void TFT_eSprite::drawPixel(_CoordsType x, _CoordsType y, uint32_t color)
		{
		  // Range checking
		...
		*************************************************************************************x*/
		void TFT_eSprite::drawLine(_CoordsType x0, _CoordsType y0, _CoordsType x1, _CoordsType y1, uint32_t color)
		{
		  if (!_created ) return;
		...
		*************************************************************************************x*/
		void TFT_eSprite::drawFastVLine(_CoordsType x, _CoordsType y, _CoordsType h, uint32_t color)
		{
		...
		*************************************************************************************x*/
		void TFT_eSprite::drawFastHLine(_CoordsType x, _CoordsType y, _CoordsType w, uint32_t color)
		{
		...
		*************************************************************************************x*/
		void TFT_eSprite::fillRect(_CoordsType x, _CoordsType y, _CoordsType w, _CoordsType h, uint32_t color)
		{
		  if (!_created ) return;
...
/*		  uint16_t uniCode = decodeUTF8(utf8);
		...
		//<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
		  ...
		  else
		  {
			if(utf8 == '\n') {
			  cursor->X(0);
			  cursor->MoveY((_CoordsType)_textsize() * (_CoordsType)pgm_read_byte(&gfxFont->yAdvance)) ;
			} else {
			  ...
			  if((w > 0) && (h > 0)) { // Is there an associated bitmap?
				_CoordsType xo = (int8_t)pgm_read_byte(&glyph->xOffset);
			  ...
			  cursor->MoveX((_CoordsType)pgm_read_byte(&glyph->xAdvance) * (_CoordsType)_textsize());
			}
		  }
		#endif // LOAD_GFXFF
		//<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
*/
		...
		*************************************************************************************x*/
		void TFT_eSprite::drawChar(_CoordsType x, _CoordsType y, uint16_t c, uint32_t color, uint32_t bg, uint8_t size)
		{
/*		  if (!_created ) return;
		...
		//>>>>>>>>>>>>>>>>>>>>>>>>>>>
			  ...
			  _CoordsType  w  = pgm_read_byte(&glyph->width),
					   h  = pgm_read_byte(&glyph->height);
					   //xa = pgm_read_byte(&glyph->xAdvance);
			  _CoordsType   xo = pgm_read_byte(&glyph->xOffset),
					   yo = pgm_read_byte(&glyph->yOffset);
			  _CoordsType  xx, yy, bits=0, bit=0;
			  _CoordsType  xo16 = 0, yo16 = 0;
			  ...
			  _CoordsType hpc = 0; // Horizontal foreground pixel count
			  for(yy=0; yy<h; yy++) {
				for(xx=0; xx<w; xx++) {
				  if(bit == 0) {
...
*/
		...
		*************************************************************************************x*/
		...
		_CoordsType TFT_eSprite::drawChar(uint16_t uniCode, _CoordsType x, _CoordsType y)
		{
//		  return drawChar(uniCode, x, y, textfont);
		  ...
		  // Any UTF-8 decoding must be done before calling drawChar()
		_CoordsType TFT_eSprite::drawChar(uint16_t uniCode, _CoordsType x, _CoordsType y, uint8_t font)
		{
/*		  if (!_created ) return 0;
...
/*		  if (code < 0x21)
		  ...
		  if (found)
		  {
			...
			for (_CoordsType y = 0; y < this->gHeight[gNum]; y++)
			{
			  this->fontFile->read(pbuffer, this->gWidth[gNum]);
			  for (_CoordsType x = 0; x < this->gWidth[gNum]; x++)
			  {
...
*/		}
		...
		*************************************************************************************x*/
		int16_t TFT_eSprite::printToSprite(_CoordsType x, _CoordsType y, uint16_t index)
		{
		  bool newSprite = !_created;
